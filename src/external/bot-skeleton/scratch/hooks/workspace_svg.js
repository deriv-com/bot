 ...
 * @private
 */
window.Blockly.WorkspaceSvg.prototype.onMouseDown_ = function (e) {
    // Bubble mousedown event up for some Core elements to react correctly.
    if (e instanceof MouseEvent) {
        window.Blockly.derivWorkspace.cachedParentSvg_.dispatchEvent(new e.constructor(e.type, e));
    }
...
window.Blockly.WorkspaceSvg.prototype.addBlockNode = function (block_node) {
    ...
    const block = window.Blockly.Xml.domToBlock(block_node, flyout.getFlyout().workspace_);
...
window.Blockly.WorkspaceSvg.getTopLevelWorkspaceMetrics_ = function () {
    ...
    if (this.toolbox_) {
        if (this.toolboxPosition === window.Blockly.TOOLBOX_AT_TOP || this.toolboxPosition === window.Blockly.TOOLBOX_AT_BOTTOM) {
 ...
 * @private
 */
window.Blockly.WorkspaceSvg.prototype.showContextMenu_ = function (e) {
    ...
    menu_options.push(window.Blockly.ContextMenu.wsUndoOption(this));
    menu_options.push(window.Blockly.ContextMenu.wsRedoOption(this));

    // Option to clean up blocks.
    if (this.scrollbar) {
        menu_options.push(window.Blockly.ContextMenu.wsCleanupOption(this, top_blocks.length));
    }
    ...
    if (this.options.collapse) {
        ...
        menu_options.push(window.Blockly.ContextMenu.wsExpandOption(has_collapsed_blocks, all_blocks));
        menu_options.push(window.Blockly.ContextMenu.wsCollapseOption(has_expanded_blocks, all_blocks));
    }

    // Option to add a workspace comment.
    if (this.options.comments) {
        menu_options.push(window.Blockly.ContextMenu.workspaceCommentOption(this, e));
    }

    menu_options.push(window.Blockly.ContextMenu.wsDeleteOption(this, top_blocks));
 ...
 */
window.Blockly.WorkspaceSvg.prototype.dispose = function (should_show_loading = false) {
    const disposeFn = () => {
        // Stop rerendering.
        this.rendered = false;
        if (this.currentGesture_) {
        ...
        window.Blockly.WorkspaceSvg.superClass_.dispose.call(this);
        if (this.svgGroup_) {
        ...
        if (this.resizeHandlerWrapper_) {
            window.Blockly.unbindEvent_(this.resizeHandlerWrapper_);

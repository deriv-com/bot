...
/** *
 * Updates a window.Blockly.FieldDropdown with the passed dropdown_options. Available options:
 * - default_value       : After updating, attempt to set this value as the selected option
 * - should_trigger_event: Should an window.Blockly.Events.Change event be triggered?
 ...
 */
window.Blockly.FieldDropdown.prototype.init = function () {
    if (this.fieldGroup_) {
    ...
    /** @type {Number} */
    this.arrowY_ = 11;
    this.arrow_ = window.Blockly.utils.createSvgElement('image', {
        height: `${this.arrowSize_}px`,
        width: `${this.arrowSize_}px`,
    });
    console.log('test inside hooks', { ...window.Blockly });
    ...
    window.Blockly.FieldDropdown.superClass_.init.call(this);
    // If not in a shadow block, draw a box.
    if (!this.sourceBlock_.isShadow()) {
        this.box_ = window.Blockly.utils.createSvgElement(
            'rect',
            {
                rx: window.Blockly.BlockSvg.CORNER_RADIUS * 4,
                ry: window.Blockly.BlockSvg.CORNER_RADIUS * 4,

 ...
 */
window.Blockly.BlockSvg.prototype.updateColour = function () {
    let strokeColour = this.getColourTertiary();
    const renderShadowed = this.isShadow() && !window.Blockly.scratchBlocksUtils.isShadowArgumentReporter(this);
    ...
    if (renderShadowed && this.parentBlock_) {
        ...
        if (
            ...
            (this.inputList[0].fieldRow[0] instanceof window.Blockly.FieldColour ||
                this.inputList[0].fieldRow[0] instanceof window.Blockly.FieldColourSlider)
        ) {
            strokeColour = window.Blockly.Colours.colourPickerStroke;
        }
    }
 ...
 */
window.Blockly.BlockSvg.prototype.computeOutputPadding_ = function (input_rows) {
    // Only apply to blocks with outputs and not single fields (shadows).
    if (
        !this.getOutputShape() ||
        !this.outputConnection ||
        (this.isShadow() && !window.Blockly.scratchBlocksUtils.isShadowArgumentReporter(this))
    ) {
        return;
    }
    ...
    if (firstField) {
        ...
        if (shape === window.Blockly.OUTPUT_SHAPE_HEXAGONAL && otherShape !== window.Blockly.OUTPUT_SHAPE_HEXAGONAL) {
            const deltaHeight = firstInput.renderHeight - window.Blockly.BlockSvg.MIN_BLOCK_Y_REPORTER;
    ...
    row.paddingStart += window.Blockly.BlockSvg.SHAPE_IN_SHAPE_PADDING[shape][otherShape];
    ...
    if (lastInput.connection) {
        ...
        // https://github.com/LLK/scratch-blocks/issues/376
        if (shape === window.Blockly.OUTPUT_SHAPE_HEXAGONAL && otherShape !== window.Blockly.OUTPUT_SHAPE_HEXAGONAL) {
            const deltaHeight = lastInput.renderHeight - window.Blockly.BlockSvg.MIN_BLOCK_Y_REPORTER;
    ...
    row.paddingEnd += window.Blockly.BlockSvg.SHAPE_IN_SHAPE_PADDING[shape][otherShape];
};

/**
 * Render a list of fields starting at the specified location.
 * @param {!Array.<!window.Blockly.Field>} fieldList List of fields.
 * @param {number} cursorX X-coordinate to start the fields.
 * @param {number} cursorY Y-coordinate to start the fields.
 * @return {number} X-coordinate of the end of the field row (plus a gap).
 * @private
 */
window.Blockly.BlockSvg.prototype.renderFields_ = function (fieldList, cursorX, cursorY) {
    ...
    for (let i = 0, field; fieldList[i]; i++) {
        ...
        if (root) {
            !this.previousConnection ||
                field instanceof window.Blockly.FieldLabel ||
                field instanceof window.Blockly.FieldImage ||
                (updatedCursorX = this.RTL
                    ? Math.min(updatedCursorX, -window.Blockly.BlockSvg.INPUT_AND_FIELD_MIN_X)
                    : Math.max(updatedCursorX, window.Blockly.BlockSvg.INPUT_AND_FIELD_MIN_X));
            let py = -field.getSize().height / 2;
            this.isScratchExtension &&
                field === this.inputList[0].fieldRow[0] &&
                field instanceof window.Blockly.FieldImage &&
                this.previousConnection &&
                (py += window.Blockly.BlockSvg.GRID_UNIT);
            this.isScratchExtension &&
                !this.previousConnection &&
                this.nextConnection &&
                field instanceof window.Blockly.FieldVerticalSeparator &&
                field.setLineHeight(window.Blockly.BlockSvg.ICON_SEPARATOR_HEIGHT - window.Blockly.BlockSvg.GRID_UNIT);
            let scale = '';
            if (this.RTL) {
                px = updatedCursorX -= field.renderSep + field.renderWidth;
                py = cursorY + py;
                field.renderWidth && (updatedCursorX -= window.Blockly.BlockSvg.SEP_SPACE_X);
            } else {
                px = updatedCursorX + field.renderSep;
                py = cursorY + py;
                field.renderWidth &&
                    (updatedCursorX += field.renderSep + field.renderWidth + window.Blockly.BlockSvg.SEP_SPACE_X);
            }
            this.RTL && field instanceof window.Blockly.FieldImage && ((scale = 'scale(-1 1)'), (px += field.renderWidth));

...
window.Blockly.BlockSvg.prototype.addSelect = function () {
    if (!this.isInFlyout) {
        ...
        window.Blockly.utils.addClass(/** @type {!Element} */ (this.svgGroup_), 'blocklySelected');
    }
};
...
window.Blockly.BlockSvg.prototype.setDisabled = function (disabled) {
    if (this.disabled !== disabled) {
        window.Blockly.BlockSvg.superClass_.setDisabled.call(this, disabled);
...
window.Blockly.BlockSvg.prototype.updateDisabled = function () {
    if (this.disabled || this.getInheritedDisabled()) {
        window.Blockly.utils.addClass(this.svgGroup_, 'blocklyDisabled');
        ...
        const removed = window.Blockly.utils.removeClass(this.svgGroup_, 'blocklyDisabled');
        if (removed) {
            this.updateColour();
        }
    }
 ...
 */
window.Blockly.BlockSvg.prototype.showContextMenu_ = function (e) {
    ...
    if (this.isDeletable() && this.isMovable() && !block.isInFlyout) {
        menu_options.push(window.Blockly.ContextMenu.blockDuplicateOption(block, e));
        menu_options.push(window.Blockly.ContextMenu.blockDetachOption(block, e));
        if (this.isEditable() && this.workspace.options.comments) {
            menu_options.push(window.Blockly.ContextMenu.blockCommentOption(block));
        }

        menu_options.push(window.Blockly.ContextMenu.blockDeleteOption(block));
    ...
    if (this.isMovable()) {
        ...
        const downloadBlock = should_delete_next => {
            const xml = window.Blockly.Xml.textToDom('<xml/>');
            const block_xml = window.Blockly.Xml.blockToDom(block);
            const file_name = should_delete_next ? block.type : `${block.type}_${localize('stack')}`;

            if (should_delete_next) {
                window.Blockly.Xml.deleteNext(block_xml);
            }
    ...
    if (block.nextConnection?.targetConnection) {
        menu_options.push(window.Blockly.ContextMenu.blockEnableOption(block, e));
        menu_options.push(window.Blockly.ContextMenu.blockDisableOption(block, e));
    }
    ...
    window.Blockly.ContextMenu.show(e, menu_options, this.RTL);
    window.Blockly.ContextMenu.currentBlock = this;
};
...
window.Blockly.BlockSvg.prototype.setErrorHighlighted = function (
    ...
    if (should_be_error_highlighted) {
        ...
        window.Blockly.utils.addClass(this.svgGroup_, highlight_class);
    } else {
        window.Blockly.utils.removeClass(this.svgGroup_, highlight_class);
    }
...
window.Blockly.BlockSvg.prototype.highlightExecutedBlock = function () {
    ...
    if (!window.Blockly.utils.hasClass(this.svgGroup_, highlight_block_class)) {
        window.Blockly.utils.addClass(this.svgGroup_, highlight_block_class);
        setTimeout(() => {
            if (this.svgGroup_) {
                window.Blockly.utils.removeClass(this.svgGroup_, highlight_block_class);
            }
        }, 1505);
    }
};
...
window.Blockly.BlockSvg.prototype.blink = function () {
    ...
    window.Blockly.utils.addClass(this.svgGroup_, blink_class);

    setTimeout(() => {
        window.Blockly.utils.removeClass(this.svgGroup_, blink_class);
    }, 2000);
};
 ...
 */
window.Blockly.BlockSvg.prototype.setCollapsed = function (collapsed) {
    ...
    if (collapsed) {
        this.getIcons()
            ...
            .filter(icon => !(icon instanceof window.Blockly.ScratchBlockComment))
        ...
        if (this.type === 'procedures_defreturn' || this.type === 'procedures_defnoreturn') {
            ...
            this.appendDummyInput(COLLAPSED_INPUT_NAME)
                .appendField(new window.Blockly.FieldLabel(localize('function'), field_class))
                .appendField(new window.Blockly.FieldLabel(function_name + args, 'header__title'))
                .appendField(field_expand_icon)
                .init();
        } else {
            const text = this.toString(window.Blockly.COLLAPSE_CHARS);
            const field_label = new window.Blockly.FieldLabel(text, field_class);
    ...
    window.Blockly.BlockSvg.superClass_.setCollapsed.call(this, collapsed);
 ...
 */
window.Blockly.BlockSvg.prototype.initSvg = function () {
    ...
    if (!this.workspace.options.readOnly && !this.eventsInit_) {
        window.Blockly.bindEventWithChecks_(this.getSvgRoot(), 'mousedown', this, this.onMouseDown_);
    }

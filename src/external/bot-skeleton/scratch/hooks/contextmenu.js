 ...
 * @param {!window.Blockly.BlockSvg} block The block where the right-click originated.
 * @return {!Object} A menu option, containing text, enabled, and a callback.
 * @package
 */
window.Blockly.ContextMenu.blockDuplicateOption = function (block) {
    const enabled = !config.single_instance_blocks.includes(block.type);
    const duplicate_option = {
        callback() {
            window.Blockly.duplicate_(block);
        },
        enabled,
 ...
 * @param {!window.Blockly.WorkspaceSvg} ws The workspace where the right-click originated.
 * @param {number} numTopBlocks The number of top blocks on the workspace.
 * @return {!Object} A menu option, containing text, enabled, and a callback.
 * @package
 */
window.Blockly.ContextMenu.wsCleanupOption = function (ws, numTopBlocks) {
    return {
 ...
 *     on the workspace.
 * @param {!Array.<!window.Blockly.BlockSvg>} topBlocks The list of top blocks on the
 *     the workspace.
 * @return {!Object} A menu option, containing text, enabled, and a callback.
 * @package
 */
window.Blockly.ContextMenu.wsCollapseOption = function (hasExpandedBlocks, topBlocks) {
    return {
        enabled: hasExpandedBlocks,
        text: localize('Collapse All Blocks'),
        callback() {
            window.Blockly.ContextMenu.toggleCollapseFn_(topBlocks, true);
        },
    };
};
 ...
 * @package
 */
window.Blockly.ContextMenu.wsExpandOption = function (hasCollapsedBlocks, topBlocks) {
    return {
        enabled: hasCollapsedBlocks,
        text: localize('Expand All Blocks'),
        callback() {
            window.Blockly.ContextMenu.toggleCollapseFn_(topBlocks, false);
        },
    };
};
 ...
 * @param {!Array.<!window.Blockly.BlockSvg>} topBlocks The list of top blocks on the the workspace.
 * @param {boolean} shouldCollapse True if the blocks should be collapsed, false if they should be expanded.
 * @private
 */
window.Blockly.ContextMenu.toggleCollapseFn_ = function (blocks, shouldCollapse) {
    blocks.forEach(block => block.setCollapsed(shouldCollapse));
};

window.Blockly.ContextMenu.wsDeleteOption = function (ws, blocks) {
    // Option to delete all blocks.
    // Count the number of blocks that are deletable.
    const delete_list = window.Blockly.WorkspaceSvg.buildDeleteList_(blocks);
    ...
    const event_group = window.Blockly.utils.genUid();
    const DELAY = 10;
 ...
 * @package
 */
window.Blockly.ContextMenu.blockCommentOption = function (block) {
 ...
 * @package
 */
window.Blockly.ContextMenu.blockDetachOption = function (block) {
 ...
 * @package
 */
window.Blockly.ContextMenu.blockDisableOption = function (block) {
 ...
 * @package
 */
window.Blockly.ContextMenu.blockEnableOption = function (block) {

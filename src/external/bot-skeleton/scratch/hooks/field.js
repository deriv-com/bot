...
const FieldCheckbox = () => {
    ...
    const icon = new window.Blockly.FieldImage(true, 16, 16, getAltText(true), onCheckboxClick);

    // Required function on window.Blockly.Field instances.
    ...
    icon.setValue = function (value) {
        ...
        if (this.sourceBlock_ && window.Blockly.Events.isEnabled()) {
            window.Blockly.Events.fire(
                new window.Blockly.Events.BlockChange(this.sourceBlock_, 'field', this.name, old_value, this.getValue())
            );
        }
    };
